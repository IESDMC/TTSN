...
(function (window, document, undefined) {
  L.LatLngGraticule = L.Layer.extend({
    ...
    onAdd: function (map) {
      ...
      this._reset();
      //   console.debug(this._map._events);
    },

    onRemove: function (map) {
      L.DomUtil.remove(this._canvas);

      //   console.debug(this._reset, this);

      //bug:無法刪除onAdd時所增加的事件
    ...
    __draw: function (label) {
      ...
      if (L.Browser.canvas && map) {
        ...
        let txtHeight = 12;
        try {
          let _font_size = ctx.font.trim().split(" ")[0];
        ...
        function __draw_lat_line(self, lat_tick) {
          ...
          let spacer = self.options.showLabel && label ? txtWidth + 10 : 0;
        ...
        function __draw_lon_line(self, lon_tick) {
          ...
          let spacer = self.options.showLabel && label ? txtHeight + 5 : 0;
